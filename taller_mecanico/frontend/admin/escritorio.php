...
$req = $connect->prepare("SELECT id, title, start, end, color FROM citas");
$req->execute();
$citas = $req->fetchAll();
     ?>
    ...
    <section id="content">
        ...
        <main>
            ...
            <ul class="breadcrumbs">
                ...
                <li class="divider">/</li>
                <li><a href="#" class="active">Panel Administratico</a></li>
            </ul>
            <div class="info-data">
                <div class="card">
                    <div class="head">
                ...
                <div class="card">
                    <div class="head">
                        <div>
                                    ...
                                    <?php 
                                            $sql = "SELECT COUNT(*) total FROM mecanicos";
                                            $result = $connect->query($sql); //$pdo sería el objeto conexión
                                            $total = $result->fetchColumn();
                ...
                <div class="card">
                    <div class="head">
                        <div>
                            <?php 
                                            $sql = "SELECT SUM(monto) total FROM citas";
                                            $result = $connect->query($sql); //$pdo sería el objeto conexión
                                            $total = $result->fetchColumn();
...
$sentencia = $connect->prepare("SELECT * FROM mecanicos ORDER BY idodc DESC LIMIT 10;");
 $sentencia->execute();
$data =  array();
if($sentencia){
    ...
    $data[] = $r;
     ...
     <?php if(count($data)>0):?>
         <table id="example" class="responsive-table">
            <thead>
                <tr>
                    <th scope="col">Nuevos Mecanicos</th>
...
$sentencia = $connect->prepare("SELECT * FROM clientes ORDER BY idpa DESC LIMIT 10;");
 $sentencia->execute();
$data =  array();
if($sentencia){
  while($r = $sentencia->fetchObject()){
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#calendar').fullCalendar({
      ...
      editable: true,
      eventLimit: true, // allow "more" link when too many citas
      selectable: true,
      selectHelper: true,
